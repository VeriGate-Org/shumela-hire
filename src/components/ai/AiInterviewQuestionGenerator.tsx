      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="col-span-2">
          ...
          <input type="text" value={candidateExperience} onChange={e => setCandidateExperience(e.target.value)}
            className="w-full text-sm p-2 border border-gray-300 rounded-sm" placeholder="e.g. 5 years in backend development" />
        </div>
      </div>
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            {candidateSkills.map((s, i) => (
              <span key={i} className="inline-flex items-center gap-1 px-2 py-1 bg-gold-50 text-violet-700 text-xs rounded-sm">
                {s}
                <button onClick={() => setCandidateSkills(prev => prev.filter((_, j) => j !== i))} className="text-violet-400 hover:text-gold-600">&times;</button>
              </span>
            ))}
          </div>
        )}
      ...
      {result && result.questions && result.questions.length > 0 && (
        <div className="space-y-2">
          {result.questions.map((q, i) => (
            <div key={i} className={`border rounded-sm p-3 cursor-pointer transition-colors ${selected.has(i) ? 'border-violet-300 bg-gold-50' : 'border-gray-200 bg-gray-50'}`}
              onClick={() => toggleQuestion(i)}>
              <div className="flex items-start gap-3">
                <input type="checkbox" checked={selected.has(i)} onChange={() => toggleQuestion(i)}
                  className="mt-1 rounded border-gray-300 text-gold-600" />
