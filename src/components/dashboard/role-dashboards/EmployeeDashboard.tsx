    ...
    <div className="space-y-6 max-w-full overflow-hidden">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-lg border border-gray-200 p-5">
          ...
          <p className="text-2xl font-bold text-violet-600 mt-1">{internalJobs.length}</p>
        </div>
        <div className="bg-white rounded-lg border border-gray-200 p-5">
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <a href="/internal/jobs" className="text-sm font-medium text-violet-600 hover:text-violet-800">
            View All
          </a>
        </div>
          ...
          {internalJobs.map((job) => (
            <div key={job.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
              ...
              <div className="text-right">
                ...
                <a href={`/internal/jobs/${job.id}`} className="text-sm font-medium text-violet-600 hover:text-violet-800">
                  View Details
                </a>
              </div>
            </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <a href="/training" className="text-sm font-medium text-violet-600 hover:text-violet-800">
            View All
          </a>
        </div>
        <div className="space-y-3">
          {trainingModules.map((module) => (
            <div key={module.id} className="p-4 bg-gray-50 rounded-lg">
              ...
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div
                  className={`h-2 rounded-full ${module.status === 'completed' ? 'bg-green-500' : 'bg-violet-500'}`}
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <a href="/profile" className="p-4 bg-violet-50 rounded-lg text-center hover:bg-violet-100 transition-colors">
            <p className="font-medium text-violet-900">My Profile</p>
            <p className="text-sm text-violet-600">Update your information</p>
          </a>
          <a href="/internal/jobs" className="p-4 bg-purple-50 rounded-lg text-center hover:bg-purple-100 transition-colors">
            <p className="font-medium text-purple-900">Internal Jobs</p>
            <p className="text-sm text-purple-600">Browse opportunities</p>
          </a>
          <a href="/training" className="p-4 bg-green-50 rounded-lg text-center hover:bg-green-100 transition-colors">
