-- V003: Job ads tables for ShumelaHire (SQL Server)
...
CREATE TRIGGER trg_job_ads_updated_at ON job_ads
AFTER UPDATE AS
BEGIN
    SET NOCOUNT ON;
    UPDATE job_ads SET updated_at = GETDATE()
    FROM job_ads INNER JOIN inserted ON job_ads.id = inserted.id;
END;
GO
