...
import com.erecruitment.backend.entity.Interview;
import com.erecruitment.backend.entity.Application;
...
public interface InterviewRepository extends JpaRepository<Interview, Long>, JpaSpecificationExecutor<Interview> {
    ...
    @Query("SELECT i FROM Interview i WHERE i.scheduledDate > :now AND i.status IN ('SCHEDULED', 'CONFIRMED') ORDER BY i.scheduledDate ASC")
    List<Interview> findUpcomingInterviews(@Param("now") LocalDateTime now);

    // Find interviews for today
    @Query("SELECT i FROM Interview i WHERE DATE(i.scheduledDate) = DATE(:date) ORDER BY i.scheduledDate ASC")
    List<Interview> findInterviewsForDate(@Param("date") LocalDateTime date);

    // Find interviews needing reminders
    @Query("SELECT i FROM Interview i WHERE i.scheduledDate > :now AND i.scheduledDate <= :reminderTime " +
    ...
    @Query("SELECT i FROM Interview i WHERE i.scheduledDate < :cutoffTime AND i.status IN ('SCHEDULED', 'CONFIRMED')")
    ...
    @Query("SELECT AVG(i.technicalScore) FROM Interview i WHERE i.technicalScore IS NOT NULL")
    Double findAverageTechnicalScore();

    @Query("SELECT AVG(i.communicationScore) FROM Interview i WHERE i.communicationScore IS NOT NULL")
    Double findAverageCommunicationScore();

    @Query("SELECT AVG(i.culturalFitScore) FROM Interview i WHERE i.culturalFitScore IS NOT NULL")
    ...
    @Query("SELECT i.interviewType, COUNT(i) FROM Interview i GROUP BY i.interviewType ORDER BY COUNT(i) DESC")
    List<Object[]> findInterviewCountByType();

    @Query("SELECT i.recommendation, COUNT(i) FROM Interview i WHERE i.recommendation != 'PENDING' GROUP BY i.recommendation")
    List<Object[]> findInterviewCountByRecommendation();

    @Query("SELECT DATE(i.scheduledDate), COUNT(i) FROM Interview i WHERE i.scheduledDate >= :fromDate " +
    ...
    @Query("SELECT i.interviewerEmail, COUNT(i), AVG(i.rating) FROM Interview i WHERE i.rating IS NOT NULL " +
    ...
    @Query("SELECT i FROM Interview i WHERE i.rating >= :minRating ORDER BY i.rating DESC, i.scheduledDate DESC")
    List<Interview> findHighRatedInterviews(@Param("minRating") Integer minRating);

    // Find interviews with detailed feedback
    @Query("SELECT i FROM Interview i WHERE i.feedback IS NOT NULL AND LENGTH(TRIM(i.feedback)) > 0 " +
           "ORDER BY i.scheduledDate DESC")
    List<Interview> findInterviewsWithFeedback();

    // Calendar view - find interviews in date range
    @Query("SELECT i FROM Interview i WHERE i.scheduledDate BETWEEN :startDate AND :endDate " +
    ...
    @Query("SELECT i FROM Interview i WHERE i.status = 'COMPLETED' AND " +
    ...
    @Query("SELECT i.interviewerName, COUNT(i), AVG(i.rating), AVG(i.technicalScore), " +
           "AVG(i.communicationScore), AVG(i.culturalFitScore) FROM Interview i " +
    ...
    @Query("SELECT AVG(i.durationMinutes) FROM Interview i WHERE i.durationMinutes IS NOT NULL")
